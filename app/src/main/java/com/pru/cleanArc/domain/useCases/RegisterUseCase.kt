...
class RegisterUseCase @Inject constructor(
    ...
    operator fun invoke(authenticationPayloadDto: AuthenticationPayloadDto): Flow<Resource<Int>> = flow<Resource<Int>> {
        emit(Resource.Loading())
        kotlin.runCatching {
            cleanArchiRepository.register(authenticationPayloadDto)
        }.onSuccess { result ->
            if (result.isError == false) {
                ...
                emit(Resource.Error(result.message ?: "Registration is failed."))
            }
        }.onFailure {
